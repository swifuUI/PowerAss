...
Page({
	...
	_loadList: async function () {
		...
		try {
			...
			await cloudHelper.callCloudSumbit('activity/list/by/day', params, opts).then(res => {
				this.setData({
					list: res.data,
					isLoad: true
				});
	...
	_loadHasList: async function () {
		let params = {
			fromDay: timeHelper.time('Y-M-D')
		}
		let opts = {
			title: 'bar'
		}
		try {
			await cloudHelper.callCloudSumbit('activity/list/has/day', params, opts).then(res => {
				this.setData({
					hasDays: res.data,
				});
			});
