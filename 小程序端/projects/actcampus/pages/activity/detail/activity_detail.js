...
Page({
	...
	_loadDetail: async function () {
		...
		activity.activityObj = JSON.parse(activity.activityObj);
		activity.activityAddressGeo = JSON.parse(activity.activityAddressGeo);
	...
	bindJoinTap: async function (e) {
		...
		wx.navigateTo({
			url: '../join/activity_join?id=' + this.data.id,
		});
	},
	...
	bindOpenMapTap: function (e) {
		...
		pageHelper.openMap(address, geo);
	},

	onShareAppMessage: function (res) {
		return {
			title: this.data.activity.activityTitle,
			imageUrl: this.data.activity.activityObj.cover[0]
		}
	}
})