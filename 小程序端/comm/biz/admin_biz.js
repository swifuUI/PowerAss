...
class AdminBiz extends BaseBiz {
	...
	static async adminLogin(that, name, password) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/login', params, opt).then(res => {
				if (res && res.data && res.data.name)
					cacheHelper.set(constants.CACHE_ADMIN, res.data, res.data.expire);
	...
	//  登录状态判定 flag=mgr/super
	static isAdmin(that, flag = '') {
		...
		else if (flag == 'mgr' && admin.type != 1 && admin.type != 0) {
			return wx.showModal({
				title: '',
				content: '此功能需要管理员操作',
				showCancel: false,
