	...
	if (geo && geo.latitude)
		...
		wx.setClipboardData({
			...
			success(res) {
				wx.getClipboardData({
					success(res) {
						showNoneToast('已复制到剪贴板，请在地图APP里查询');
					}
				})
			}
		});
...
function getPID() {
	...
	if (!PID) route = getParentPageURL();

	PID = route.replace('/projects/', '');
	PID = PID.split('/')[0];
 ...
 * @param {*} idName 主键名
 */
function modifyPrevPageListNodeObject(id, vals, deep = 2, listName = 'dataList', idName = 'id') {
	let prevPage = getPrevPage(deep);
	if (!prevPage) return;
 ...
 * @param {*} idName 
 */
function delListNode(id, list, idName = 'id') {
	if (!list || !Array.isArray(list)) return false;
	let pos = list.findIndex(item => item[idName] == id);
	if (pos > -1) {
		list.splice(pos, 1);
